...
function TodoList() {
  ...
  // 아래 useState(초기값) 은 실제로는 API Server 연동을 통해 확보하지마 지금은 직접 입력한다 ( JSON Array )
  ...
  const addTodo = () => {
    if (newTodo.trim()) {
      ...
      setTodos([...todos, todo]); // 기존 배열에 새 항목 추가한 새 배열을 생성해서 할당
      setNewTodo(""); // 입력창 초기화
    }
  };
  ...
  const deleteTodo = (id) => {
    ...
    // filter 함수는 todo.id != id이 true 이면 새 배열 요소로 추가
    ...
    <div className="todo-list">
      ...
      <div className="add-todo">
        <input
          ...
          onChange={(e) => setNewTodo(e.target.value)} // 입력 요소 value가 change 변경될 때 react hook의 함수로 상태를 변경 -> 앱에 리렌더링
          // key를 눌렸을 때 발생하는 이벤트

          // keyDown 이벤트 발생시 실행될 화살표 함수 arrow function를 등록 binding
          // addTodo가 아니라 addTodo() 로 명시한 이유는 엔터키 이벤트 발생시에 바로 호출 즉 실행되어 todo를 추가하기 위해 즉 구현부 내에서는 실행을 해야 하므로 반드시 ()를 명시해야 됨
      ...
      <ul className="todo-items">
        {todos.map((todo) => (
          ...
          >
            ...
            {/* 삭제 버튼이 클릭되면(클릭이벤트발생시) 실행될 화살표 함수를 등록
      ...
      {/* to do liset가 비어 있을 때 아래를 보이도록 조건 처리한다 */}
