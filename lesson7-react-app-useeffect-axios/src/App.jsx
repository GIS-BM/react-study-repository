...
// function App(){} // 아래에는 동일한 표현
const App = () => {
  // useState hook 으로 ajax 요청 중 인지를 저장
  ...
  // useEffect hook 을 이용해 lifecycle의 mount 단계에서 API Server의 상품 리스트를 가져온다
  // 의존성 배열은 빈 배열을 할당해서 마운트 시점에 상품리스트 데이터 로딩이 한번만 수행되도록 한다.
  useEffect(() => {
  ...
  const addProduct = async (productData) => {
    try {
      ...
      /* 상품 등록이 성공되면 상품리스트에 추가 상품을 새배열 마지막에 추가해서 state에 할당 ->  리렌더링 -> ProductList component에 새로운 Props(상품리스트) 전달 -> 리스트 갱신 */
  ...
  return (
    <div className="app">
      ...
      {loading ? (
        ...
        // axios로 api server에서 받아온 상품리스틀를 props로 전달한다.
