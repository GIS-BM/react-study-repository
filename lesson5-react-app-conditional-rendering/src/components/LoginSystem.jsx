...
function LoginSystem() {
  ...
  // React hook, Router와 연결 [상태변수, 상태변화]
  ...
  const handleLogin = () => {
    ...
    if (name) {
      ...
      setIsLoggedIn(true); // React state hook을 이용해 loggedIn을 할당하여 리렌더링되어 보여짐
    }
  };
  // 로그아웃 처리 함수
  const handleLogout = () => {
  ...
  return (
    <div className="login-system">
      <h2>로그인 시스템</h2>
      {/* 삼항 연산자 : isLoggedIn state 값에 따라 true이면 로그인 화면 렌더링과 false
      이면 비로그인 화면 렌더링을 구분해 보여준다. */}
      {isLoggedIn ? (
